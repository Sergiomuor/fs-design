...
5. **Propagate**: the fs-design plugin carries a snapshot (`reference/DESIGN.snapshot.md`) and sentinel values in `scripts/context.mjs` + hint maps in `scripts/luz-lint.mjs` — if tokens changed, update the snapshot and flag the plugin files that need the same values (list them explicitly in the report).
