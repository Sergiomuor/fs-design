...
2. **Classify the project.**
   ...
   - **A satellite** (prototype, showcase, marketing experiment): copy both snapshots and prepend a note to DESIGN.md: ground truth lives in the platform repo (`packages/luz/src/styles.css` on main); this copy is a reference for building Luz-faithful UI outside the platform. Stamp the note with the snapshot's provenance (installed plugin version + the snapshot's own verified-on date) and one refresh instruction: after any plugin update, or whenever platform styles.css changes, re-run `init` (or `document` from a platform checkout) to resync this copy. Ask one question: does this project follow Luz strictly (product surface) or loosely (brand-adjacent, e.g. marketing)? Loose followers get the note extended: tokens are the palette, but layout/type scale may flex — say which rules bind.
3. **Offer the enforcement layer.** Ask whether to enable the luz-lint hook here (`node <plugin-root>/skills/fs-design/scripts/hook-admin.mjs on`) — recommended for product surfaces and strict-Luz satellites, optional for loose (brand-adjacent) satellites. Mention `lint --diff` as the manual alternative.
