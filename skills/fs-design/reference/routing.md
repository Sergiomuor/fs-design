...
- Nothing in flight → offer by intent: **plan something new** (`shape`), **evaluate what exists** (`critique`, `audit`), **improve what exists** (`polish`, `normalize`, `distill`, `clarify`), **teach the suite** (`feedback` — turn what a past run got wrong into persistent rules).
- Earlier fs-design work is visible in the conversation or its output was just reviewed → include `feedback` among the picks; never as the sole lead.
