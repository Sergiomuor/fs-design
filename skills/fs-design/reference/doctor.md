...
6. **Install freshness** — the version in `~/.claude/plugins/installed_plugins.json` for `fs-design@fs-design` vs `.claude-plugin/plugin.json` in the source repo (path in `~/.claude/plugins/known_marketplaces.json`), and uncommitted changes in that repo. A stale or behind cache means runs obey an old plugin — repair is commit + `claude plugin update fs-design@fs-design` (compare versions/content, not the cache's `gitCommitSha`, which is not refreshed on update). Also spot-check that learnings entries marked `promoted → <file>` actually appear in their destination files.
